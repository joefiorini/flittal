...
Elm.Board.Controller.make = function (_elm) {
   ...
   state) {
      return function () {
         var updateBox = F2(function (lastBox,
         _v0) {
            return function () {
               return A2($Box$Controller.step,
               $Box$Controller.SetSelected(lastBox.selectedIndex + 1),
               box);
            }();
         });
   ...
   var decode = $Board$Model.decode;
   var encode = $Board$Model.encode;
   var entersEditMode = function (update) {
      return $Debug.log("entersEditMode")(function () {
         switch (update.ctor)
         {case "BoxAction":
            return $Box$Controller.entersEditMode(update._0);
            case "EditingBox":
            return update._1;
   ...
   state) {
      return function () {
         ...
         var nextSelectedIndex = function (boxes) {
            ...
            F2(function (last,index) {
               return _U.cmp(index,
               last) > 0 ? index + 1 : last + 1;
            }),
            0)(A2($List.map,
            function (_) {
               return _.selectedIndex;
         ...
         var performActionOnAllBoxes = function (update) {
            return $Box$Controller.step(update);
         };
         var updateSelectedBoxes = F2(function (update,
         iterator) {
            return _U.cmp(iterator.selectedIndex,
            -1) > 0 ? A2($Box$Controller.step,
            update,
            iterator) : iterator;
         });
         var updateBoxInState = F3(function (update,
         box,
         iterator) {
            ...
            {case "BoxAction":
               switch (_v7._0.ctor)
                 {case "CancelEditingBox":
                    return function () {
                         var box$ = A2($Box$Controller.step,
                         $Box$Controller.CancelEditing,
                         _v7._0._0);
                         var boxes$ = A2(replaceBox,
                         state.boxes,
                         box$);
                         return A2($Debug.log,
                         "Canceling edit",
                    ...
                    return function () {
                         var box$ = A2($Box$Controller.step,
                         $Box$Controller.Editing(_v7._0._1),
                         _v7._0._0);
                         return A2($Debug.log,
                         "editing box",
                         _U.replace([["boxes"
                                     ,replaceBox(state.boxes)(box$)]],
                         state));
                      }();
                    case "UpdateBox":
                    return A2($Debug.log,
                      "Box.UpdateBox",
                      _U.replace([["boxes"
                                  ,replaceBox(state.boxes)(A2($Box$Controller.step,
                                  $Box$Controller.Update(_v7._0._1),
                                  _v7._0._0))]],
                      state));}
                 break;
               case "ClearBoard":
               ...
               return _U.cmp($List.length(selectedBoxes(state.boxes)),
                 ...
                 _U.replace([["connections"
                             ,$Connection$Controller.buildConnections(state.connections)($Debug.log("Selected Boxes")(selectedBoxes(state.boxes)))]],
                 state));
               ...
               case "DeselectBoxes":
               return function () {
                    var cancelEditing = $List.map($Box$Controller.step($Box$Controller.CancelEditing));
               ...
               return function () {
                    ...
                    var selectedBox = function (boxes) {
                       return A2($List.map,
                       A2(updateBoxInState,
                       $Box$Controller.SetSelected(nextSelectedIndex(boxes)),
                       box),
                       boxes);
                    };
                    var updateBoxes = function ($) {
               ...
               return function () {
                    var selectedBoxes = A2($List.filter,
                    function (b) {
                       return !_U.eq(b.selectedIndex,
                       -1);
                    },
                    state.boxes);
                    return _U.eq($List.length(selectedBoxes),
                    1) ? _U.replace([["boxes"
                                     ,replaceBox(state.boxes)($Box$Controller.step($Box$Controller.Editing(_v7._0))($List.head(selectedBoxes)))]],
                    state) : state;
                 }();
               case "MoveBox":
               return function () {
               ...
               return isEditing(state.boxes) ? state : function () {
                    var newBox = makeBox(state.nextIdentifier);
                    return A2($Debug.log,
                    "newBox",
                    _U.replace([["boxes"
                                ,A2($List._op["::"],
               ...
               return _U.replace([["connections"
                                  ,A3($Connection$Controller.boxMap,
                                  $Connection$Controller.connectBoxes,
                                  state.boxes,
                                  state.connections)]],
                 state);
               case "ResizeBox":
               ...
               return function () {
                    ...
                    return _U.cmp(box.selectedIndex,
                    -1) > 0 ? state : A2($Debug.log,
                    "selecting box",
                    _U.replace([["boxes"
                                ,updateBoxes(state.boxes)]],
                    state));
               ...
               return function () {
                    ...
                    _v7._0,
                    state.boxes);
                    var updateBoxes = function (boxes) {
                       return A2($List.map,
                       A2(updateBoxInState,
                       $Box$Controller.SetSelected(nextSelectedIndex(boxes)),
                       box),
   ...
   var EditingSelectedBox = function (a) {
      return {ctor: "EditingSelectedBox"
             ,_0: a};
   };
   ...
   var buildSelectAction = function (event) {
      return function () {
         var boxIdM = $DomUtils.extractBoxId(event.id);
         return function () {
            switch (boxIdM.ctor)
            {case "Err":
               return A2($Debug.log,
   ...
   var widgets = F2(function (channel,
   board) {
      return function () {
         var boxChannel = A2($LocalChannel.localize,
         function (a) {
            return $Debug.log("BoxAction")(BoxAction(a));
         },
         channel);
         return A2($List.concatMap,
         $Basics.identity,
         _L.fromArray([A2($List.map,
                      $Box$Controller.view(boxChannel),
                      board.boxes)
                      ,renderConnections(board.connections)]));
      }();
   });
   var NoOp = {ctor: "NoOp"};
   var buildEditingAction = function (id) {
      return function () {
         var boxIdM = $DomUtils.extractBoxId(id);
         return function () {
            switch (boxIdM.ctor)
            {case "Err": return NoOp;
               case "Ok": return A2(EditingBox,
                 boxIdM._0,
                 true);}
            _U.badCase($moduleName,
   ...
   height) {
      return $Html.div(_L.fromArray([$Html$Attributes.style(_L.fromArray([$DomUtils.styleProperty("height")($Geometry$Types.toPx(height))]))
                                    ...
                                    ,A3($Html$Events.on,
                                    "dblclick",
                                    $DomUtils.getTargetId,
                                    function (v) {
                                       return $LocalChannel.send(channel)(buildEditingAction(v));
                                    })
                                    ,A3($Html$Events.on,
                                    "mousedown",
                                    $DomUtils.getMouseSelectionEvent,
                                    function (v) {
                                       return $LocalChannel.send(channel)(buildSelectAction(v));
                                    })]))(A2(widgets,
      channel,
      model));
   });
   ...
   _elm.Board.Controller.values = {_op: _op
                                  ...
                                  ,EditingSelectedBox: EditingSelectedBox
                                  ,SelectBox: SelectBox
                                  ...
                                  ,buildSelectAction: buildSelectAction
                                  ,buildEditingAction: buildEditingAction
                                  ,widgets: widgets
                                  ,actions: actions
...
Elm.Board = Elm.Board || {};
Elm.Board.Model = Elm.Board.Model || {};
Elm.Board.Model.make = function (_elm) {
   "use strict";
   _elm.Board = _elm.Board || {};
   _elm.Board.Model = _elm.Board.Model || {};
   if (_elm.Board.Model.values)
   return _elm.Board.Model.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Board.Model",
   $Box$Model = Elm.Box.Model.make(_elm),
   $Connection$Model = Elm.Connection.Model.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   $Json$Encode = Elm.Json.Encode.make(_elm),
   $List = Elm.List.make(_elm);
   var encode = function (board) {
      return function () {
         var encodedConnections = A2($List.map,
         $Connection$Model.encode,
         board.connections);
         var encodedBoxes = A2($List.map,
         $Box$Model.encode,
         board.boxes);
         return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                                  ,_0: "boxes"
                                                  ,_1: $Json$Encode.list(encodedBoxes)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "connections"
                                                  ,_1: $Json$Encode.list(encodedConnections)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "nextIdentifier"
                                                  ,_1: $Json$Encode.$int(board.nextIdentifier)}]));
      }();
   };
   ...
   var decode = A4($Json$Decode.object3,
   Model,
   A2($Json$Decode._op[":="],
   "boxes",
   $Json$Decode.list($Box$Model.decode)),
   A2($Json$Decode._op[":="],
   "connections",
   $Json$Decode.list($Connection$Model.decode)),
   A2($Json$Decode._op[":="],
   "nextIdentifier",
   $Json$Decode.$int));
   _elm.Board.Model.values = {_op: _op
                             ,Model: Model
                             ,encode: encode
                             ,decode: decode};
   return _elm.Board.Model.values;
...
Elm.Box.Controller.make = function (_elm) {
   ...
   var NoOp = {ctor: "NoOp"};
   var labelField = F3(function (channel,
   box,
   label) {
      return function () {
   ...
   var onKeyDown = F2(function (channel,
   box) {
      return function () {
         var checkKeyCode = function (keyCode) {
            return function () {
               switch (keyCode)
               {case 13: return A2(EditingBox,
                    box,
                    false);
                  case 27:
                  return CancelEditingBox(box);}
               return NoOp;
            }();
         };
         ...
         $Html$Events.keyCode,
         function (v) {
            return $Debug.log("box:keydown")($LocalChannel.send(channel)(checkKeyCode(v)));
         });
      }();
   });
   ...
   _elm.Box.Controller.values = {_op: _op
                                ...
                                ,encode: encode
                                ,onKeyDown: onKeyDown
                                ,keyCodeAndValue: keyCodeAndValue
...
Elm.Box = Elm.Box || {};
Elm.Box.Model = Elm.Box.Model || {};
Elm.Box.Model.make = function (_elm) {
   "use strict";
   _elm.Box = _elm.Box || {};
   _elm.Box.Model = _elm.Box.Model || {};
   if (_elm.Box.Model.values)
   return _elm.Box.Model.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Box.Model",
   $Basics = Elm.Basics.make(_elm),
   $Geometry$Types = Elm.Geometry.Types.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   $Json$Encode = Elm.Json.Encode.make(_elm),
   ...
   var encode = function (box) {
      return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                               ,_0: "position"
                                               ,_1: $Geometry$Types.encodePoint(box.position)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "size"
                                               ,_1: $Geometry$Types.encodeSize(box.size)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "key"
                                               ,_1: $Json$Encode.$int(box.key)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "label"
                                               ,_1: $Json$Encode.string(box.label)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "originalLabel"
                                               ,_1: $Json$Encode.string(box.originalLabel)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "isEditing"
                                               ,_1: $Json$Encode.bool(box.isEditing)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "isDragging"
                                               ,_1: $Json$Encode.bool(box.isDragging)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "selectedIndex"
   ...
   var decode = function () {
      ...
      _op["~="] = F2(function (property,
      decoder) {
         return $Json$Decode.decodeValue(A2($Json$Decode._op[":="],
         property,
         decoder));
      });
      return A2(apply,
      ...
      A2(extract,
      "position",
      $Geometry$Types.decodePoint)),
      A2(extract,
      "size",
      $Geometry$Types.decodeSize)),
      A2(extract,
      "label",
      $Json$Decode.string)),
      A2(extract,
...
Elm.Connection.Controller.make = function (_elm) {
   ...
   connections) {
      ...
      function (c) {
         return function () {
            var endBox = $List.head(A2($List.filter,
            function (b) {
               return _U.eq(b.key,c.endBox);
            },
            boxes));
            var startBox = $List.head(A2($List.filter,
            function (b) {
               return _U.eq(b.key,
               c.startBox);
            },
            boxes));
            return A2(f,startBox,endBox);
         }();
      },
      connections);
   });
   ...
   var leftPort = function (_v2) {
      return function () {
         return function () {
            ...
            x = $._0,
            y = $._1;
            return $Connection$Model.Left({ctor: "_Tuple2"
                                          ,_0: x
                                          ,_1: y + midPoint(h)});
         }();
      }();
   };
   var bottomPort = function (_v4) {
      return function () {
         return function () {
            var $ = _v4.size,
            w = $._0,
   ...
   var topPort = function (_v6) {
      return function () {
         return function () {
            ...
            x = $._0,
            y = $._1;
            return $Connection$Model.Top({ctor: "_Tuple2"
                                         ,_0: x + midPoint(w)
                                         ,_1: y});
         }();
      }();
   ...
   var connectBoxes = F2(function (startBox,
   endBox) {
      return {_: {}
             ,endBox: endBox.key
             ,endPort: function (_) {
   ...
   var offsetMidpoint = function (x) {
      return !_U.eq(A2($Basics._op["%"],
      x,
      2),
      0) ? x - 1 : x;
   };
   var endpointHeight = 10;
   var endpointMidY = offsetMidpoint(endpointHeight / 2 | 0);
   var endpointWidth = 10;
   var endpointMidX = offsetMidpoint(endpointWidth / 2 | 0);
   var drawEndpoint = function (p) {
      return function () {
         var points = function () {
            switch (p.ctor)
            {case "Bottom":
               switch (p._0.ctor)
                 {case "_Tuple2":
                    return {ctor: "_Tuple2"
                           ,_0: p._0._0 - endpointMidX
                           ,_1: p._0._1};}
                 break;
               case "Left": switch (p._0.ctor)
                 {case "_Tuple2":
                    return {ctor: "_Tuple2"
                           ,_0: p._0._0 - endpointWidth
                           ,_1: p._0._1 - endpointMidY};}
                 break;
               case "Right": switch (p._0.ctor)
                 {case "_Tuple2":
                    return {ctor: "_Tuple2"
                           ,_0: p._0._0
                           ,_1: p._0._1 - endpointMidY};}
                 break;
               case "Top": switch (p._0.ctor)
                 {case "_Tuple2":
                    return {ctor: "_Tuple2"
                           ,_0: p._0._0 - endpointMidX
                           ,_1: p._0._1 - endpointHeight};}
                 break;}
            _U.badCase($moduleName,
         ...
         _L.fromArray([$Html$Attributes.style(_L.fromArray([A2($DomUtils.styleProperty,
                                                           ...
                                                           "background-color",
                                                           "black")
                                                           ,$DomUtils.styleProperty("width")($Geometry$Types.toPx(endpointWidth))
                                                           ,$DomUtils.styleProperty("height")($Geometry$Types.toPx(endpointHeight))
                                                           ,$DomUtils.styleProperty("top")($Basics.snd($Geometry$Types.toPxPoint(points)))
                                                           ,$DomUtils.styleProperty("left")($Basics.fst($Geometry$Types.toPxPoint(points)))]))]),
         _L.fromArray([]));
      }();
   };
   ...
   var renderConnection = function (connection) {
      return $Html.div(_L.fromArray([$Html$Attributes.$class("connection")]))(A2($List._op["::"],
      drawEndpoint(connection.endPort),
      A2($List.map,
      drawSegment,
      connection.segments)));
   };
   _elm.Connection.Controller.values = {_op: _op
                                       ...
                                       ,endpointHeight: endpointHeight
                                       ,offsetMidpoint: offsetMidpoint
                                       ,endpointMidX: endpointMidX
                                       ,endpointMidY: endpointMidY
                                       ...
                                       ,connectBoxesFold: connectBoxesFold};
   return _elm.Connection.Controller.values;
};
Elm.Connection = Elm.Connection || {};
Elm.Connection.Model = Elm.Connection.Model || {};
Elm.Connection.Model.make = function (_elm) {
   "use strict";
   _elm.Connection = _elm.Connection || {};
   _elm.Connection.Model = _elm.Connection.Model || {};
   if (_elm.Connection.Model.values)
   return _elm.Connection.Model.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Connection.Model",
   $Basics = Elm.Basics.make(_elm),
   $Box$Model = Elm.Box.Model.make(_elm),
   $Geometry$Types = Elm.Geometry.Types.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Result = Elm.Result.make(_elm);
   var encodeLineLayout = function (layout) {
      return function () {
         switch (layout.ctor)
         {case "Horizontal":
            return $Json$Encode.string("Horizontal");
            case "Vertical":
            return $Json$Encode.string("Vertical");}
         _U.badCase($moduleName,
         "between lines 87 and 89");
      }();
   };
   var encodeSegment = function (segment) {
      return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                               ,_0: "position"
                                               ,_1: $Geometry$Types.encodePoint(segment.position)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "size"
                                               ,_1: $Geometry$Types.encodeSize(segment.size)}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "layout"
                                               ,_1: encodeLineLayout(segment.layout)}]));
   };
   var encodePort = function (portLocation) {
      return function () {
         var $ = function () {
            switch (portLocation.ctor)
            {case "Bottom":
               return {ctor: "_Tuple2"
                      ,_0: "bottom"
                      ,_1: portLocation._0};
         ...
         return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                                  ,_0: "location"
                                                  ,_1: $Json$Encode.string(location)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "point"
                                                  ,_1: $Geometry$Types.encodePoint(point)}]));
      }();
   };
   var encode = function (connection) {
      return function () {
         var encodedSegments = A2($List.map,
         encodeSegment,
         connection.segments);
         return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                                  ,_0: "segments"
                                                  ,_1: $Json$Encode.list(encodedSegments)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "startPort"
                                                  ,_1: encodePort(connection.startPort)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "endPort"
                                                  ,_1: encodePort(connection.endPort)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "startBox"
                                                  ,_1: $Json$Encode.$int(connection.startBox)}
                                                 ,{ctor: "_Tuple2"
                                                  ,_0: "endBox"
                                                  ,_1: $Json$Encode.$int(connection.endBox)}]));
      }();
   };
   ...
   var decodePort = A2($Json$Decode.customDecoder,
   portRawDecoder,
   function (_v6) {
      return function () {
         return function () {
            var _v8 = _v6.location;
            switch (_v8)
            {case "bottom":
               return $Result.Ok(Bottom(_v6.point));
               case "left":
               return $Result.Ok(Left(_v6.point));
               case "right":
               return $Result.Ok(Right(_v6.point));
               case "top":
               return $Result.Ok(Top(_v6.point));}
   ...
   var ConnectionPort = F3(function (a,
   b,
   c) {
      return {_: {}
             ,end: b
   ...
   var EndStart = {ctor: "EndStart"};
   var StartEnd = {ctor: "StartEnd"};
   var Horizontal = {ctor: "Horizontal"};
   var Vertical = {ctor: "Vertical"};
   var decodeLineLayout = A2($Json$Decode.customDecoder,
   $Json$Decode.string,
   function (layout) {
      return function () {
         switch (layout)
         {case "Horizontal":
            return $Result.Ok(Horizontal);
            case "Vertical":
            return $Result.Ok(Vertical);}
         return $Result.Err(A2($Basics._op["++"],
         "layout value of \"",
         A2($Basics._op["++"],
         layout,
   ...
   var decodeSegment = A4($Json$Decode.object3,
   Line,
   A2($Json$Decode._op[":="],
   "position",
   $Geometry$Types.decodePoint),
   ...
   "endBox",
   $Json$Decode.$int));
   _elm.Connection.Model.values = {_op: _op
                                  ,Vertical: Vertical
                                  ...
                                  ,portRawDecoder: portRawDecoder
                                  ,decodePort: decodePort
                                  ,encode: encode
                                  ,encodeLineLayout: encodeLineLayout
                                  ,decodeLineLayout: decodeLineLayout
...
Elm.DomUtils.make = function (_elm) {
   ...
   var on = $Native$Custom$Html.on;
   var setFocus = $Native$Custom$Html.setFocus;
...
Elm.Geometry.Types.make = function (_elm) {
   ...
   var encodeSize = function (size) {
      return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                               ,_0: "width"
                                               ,_1: $Json$Encode.$int($Basics.fst(size))}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "height"
   ...
   var encodePoint = function (point) {
      return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                               ,_0: "x"
                                               ,_1: $Json$Encode.$int($Basics.fst(point))}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "y"
                                               ,_1: $Json$Encode.$int($Basics.snd(point))}]));
   };
   var toPx = function (n) {
      return A2($Basics._op["++"],
      $Basics.toString(n),
...
Elm.Json = Elm.Json || {};
Elm.Json.Ext = Elm.Json.Ext || {};
Elm.Json.Ext.make = function (_elm) {
   "use strict";
   _elm.Json = _elm.Json || {};
   _elm.Json.Ext = _elm.Json.Ext || {};
   if (_elm.Json.Ext.values)
   return _elm.Json.Ext.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $moduleName = "Json.Ext",
   $Native$Json = Elm.Native.Json.make(_elm);
   var map2 = $Native$Json.decodeObject2;
   _elm.Json.Ext.values = {_op: _op
                          ,map2: map2};
   return _elm.Json.Ext.values;
};
...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $Style$Color = Elm.Style.Color.make(_elm),
   $TimeMachine = Elm.TimeMachine.make(_elm),
      ...
      return function () {
         ...
         {case "BoardUpdate":
            return function () {
                 ...
                 _v0._0,
                 state.currentBoard);
                 var recordedHistory = A2($TimeMachine.record,
                 updatedBoard,
                 state.boardHistory);
   ...
   var entersEditMode = function (update) {
      return function () {
         switch (update.ctor)
         {case "BoardUpdate":
            return $Board$Controller.entersEditMode(update._0);}
         return false;
      }();
   };
   ...
   var HydrateAppState = function (a) {
      return {ctor: "HydrateAppState"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   ...
   var extractAppState = function (result) {
      return function () {
         switch (result.ctor)
         {case "Err":
   ...
   var mkState = function (board) {
      return {_: {}
             ,boardHistory: $TimeMachine.initialize($Board$Controller.startingState)
             ,currentBoard: board};
   };
   var decodeAppState = A2($Json$Decode.object1,
   mkState,
   A2($Json$Decode._op[":="],
   "currentBoard",
   $Board$Controller.decode));
   var encodeAppState = function (state) {
      return $Json$Encode.object(_L.fromArray([{ctor: "_Tuple2"
                                               ,_0: "currentBoard"
                                               ,_1: $Board$Controller.encode(state.currentBoard)}]));
   };
   var routesMap = function (routeName) {
      return function () {
         var url = function () {
   ...
   var deserializedState = function () {
      var loadedState$ = A3($Signal.keepIf,
      F2(function (x,y) {
         return !_U.eq(x,y);
      })(""),
      $Json$Encode.encode(0)(encodeAppState(startingState)),
      loadedState);
      var deserializeAppState = $Json$Decode.decodeString(decodeAppState);
      return A2($Signal._op["<~"],
      function ($) {
         return HydrateAppState(extractAppState(deserializeAppState($)));
      },
      loadedState$);
   }();
   ...
   var globalKeyDown = _P.portIn("globalKeyDown",
   _P.incomingSignal(function (v) {
   ...
   var userInput = $Signal.mergeMany(_L.fromArray([drop
                                                  ,dragstart
                                                  ,dragend]));
   var state = A3($Signal.foldp,
   step,
   startingState,
   $Signal.mergeMany(_L.fromArray([$Signal.subscribe(updates)
                                  ,deserializedState
                                  ,A2($Signal.map,
                                  globalKeyboardShortcuts,
                                  $Mousetrap.keydown)
                                  ,A2($Signal._op["<~"],
                                  convertDragOperation,
                                  $Signal.mergeMany(_L.fromArray([drop
                                                                 ,dragstart])))])));
   ...
   routeHandler),
   $Window.dimensions);
   var serializeState = _P.portOut("serializeState",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   function () {
...
Elm.Mousetrap = Elm.Mousetrap || {};
Elm.Mousetrap.make = function (_elm) {
   "use strict";
   _elm.Mousetrap = _elm.Mousetrap || {};
   if (_elm.Mousetrap.values)
   return _elm.Mousetrap.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Mousetrap",
   $Native$Mousetrap = Elm.Native.Mousetrap.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var keypress = $Native$Mousetrap.keypress;
   var keyup = $Native$Mousetrap.keyup;
   var keydown = $Native$Mousetrap.keydown;
   _elm.Mousetrap.values = {_op: _op
                           ,keydown: keydown
                           ,keyup: keyup
                           ,keypress: keypress};
   return _elm.Mousetrap.values;
...
},{"mousetrap":"mousetrap","mousetrap-additions":"mousetrap-additions"}],"mousetrap-additions":[function(require,module,exports){
var additions = {};
...
Elm.Partials.Footer.make = function (_elm) {
   "use strict";
   _elm.Partials = _elm.Partials || {};
   _elm.Partials.Footer = _elm.Partials.Footer || {};
   if (_elm.Partials.Footer.values)
...
Elm.Partials = Elm.Partials || {};
Elm.Partials.Header = Elm.Partials.Header || {};
Elm.Partials.Header.make = function (_elm) {
   "use strict";
   _elm.Partials = _elm.Partials || {};
   _elm.Partials.Header = _elm.Partials.Header || {};
   if (_elm.Partials.Header.values)
...
Elm.Routes.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var sendToPort = function (routeSignal) {
...
Elm.TimeMachine.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $TimeMachine$History = Elm.TimeMachine.History.make(_elm),
   $TimeMachine$Travel = Elm.TimeMachine.Travel.make(_elm);
   var travelForward = $TimeMachine$Travel.forward;
   var travelBackward = $TimeMachine$Travel.backward;
   var record = $TimeMachine$History.record;
   var initialize = A2($Basics.flip,
   record,
   $TimeMachine$History.$default);
   _elm.TimeMachine.values = {_op: _op
                             ,record: record
                             ,travelBackward: travelBackward
                             ,travelForward: travelForward
                             ,initialize: initialize};
   return _elm.TimeMachine.values;
};
Elm.TimeMachine = Elm.TimeMachine || {};
Elm.TimeMachine.History = Elm.TimeMachine.History || {};
Elm.TimeMachine.History.make = function (_elm) {
   "use strict";
   _elm.TimeMachine = _elm.TimeMachine || {};
   _elm.TimeMachine.History = _elm.TimeMachine.History || {};
   if (_elm.TimeMachine.History.values)
   return _elm.TimeMachine.History.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "TimeMachine.History",
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm);
   var record = F2(function (entry,
   history) {
      return function () {
         var entries$ = A2($List._op["::"],
         entry,
         history.entries);
   ...
   _elm.TimeMachine.History.values = {_op: _op
                                     ,History: History
                                     ,initial: initial
                                     ,$default: $default
                                     ,record: record};
...
Elm.TimeMachine = Elm.TimeMachine || {};
Elm.TimeMachine.Travel = Elm.TimeMachine.Travel || {};
Elm.TimeMachine.Travel.make = function (_elm) {
   "use strict";
   _elm.TimeMachine = _elm.TimeMachine || {};
   _elm.TimeMachine.Travel = _elm.TimeMachine.Travel || {};
   if (_elm.TimeMachine.Travel.values)
   return _elm.TimeMachine.Travel.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "TimeMachine.Travel",
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm);
   var forward = function (history) {
      return function () {
         var pointer$ = _U.eq(history.pointer,
         1) ? -1 : history.pointer - 2;
         var current$ = $Array.get(pointer$)($Array.fromList(history.entries));
   ...
   var backward = function (history) {
      return function () {
         var pointer$ = history.pointer + 1;
         var current$ = $Array.get(history.pointer)($Array.fromList(history.entries));
         return _U.replace([["current"
                            ,current$]
                           ,["pointer",pointer$]],
         history);
   ...
   _elm.TimeMachine.Travel.values = {_op: _op
                                    ,backward: backward
                                    ,forward: forward};
   return _elm.TimeMachine.Travel.values;
};
