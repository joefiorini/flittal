...
decodePort =
  Decode.customDecoder portRawDecoder
    (\{location, point} ->
      case location of
        "right" -> Result.Ok <| Right point
        "left" -> Result.Ok <| Left point
        "top" -> Result.Ok <| Top point
        "bottom" -> Result.Ok <| Bottom point
        _ -> Result.Err ("port location value of \"" ++ location ++ "\" is invalid.")
    )

encode connection =
  let encodedSegments = List.map encodeSegment connection.segments
  in
     Encode.object
...
encodeLineLayout layout =
  case layout of
    Vertical -> Encode.string "Vertical"
    Horizontal -> Encode.string "Horizontal"

decodeLineLayout =
  Decode.customDecoder Decode.string (\layout ->
    case layout of
      "Vertical" -> Result.Ok Vertical
      "Horizontal" -> Result.Ok Horizontal
      _ -> Result.Err ("layout value of \"" ++ layout ++ "\" is invalid.")
    )
